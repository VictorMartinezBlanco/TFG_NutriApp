...
\begin{enumerate}
    ...
    \item To carry out a market study and competitor analysis in the field of nutrition platforms, applying knowledge from the Business Administration training. This includes identifying the main competitors, analysing their strengths and weaknesses, detecting gaps and opportunities in the market, and positioning NutriApp with innovative and differentiating features that add real value.
...
The technical heart of the project: the formalisation of the diet as a constraint satisfaction problem. After asking Alejandro which approach would be best, he explained to me that its implementation should consist of a constraint programming solver and an independent deterministic validator that audits every plan before accepting it, and he pointed out that I could also add an infeasibility diagnosis for when the requests are incompatible with each other. The engine works without any artificial intelligence component, which made it possible to verify it first with a bank of cases and then from the web itself (by creating a section to modify the parameters used to generate the plan with the engine).
...
The evaluation accompanied the whole development and only its last piece remains. So far it includes a pre-registered study on the effect of message format on translation (with a negative result that avoided building an unnecessary module), performance measurements of the engine in the production environment as the food catalogue grew, and a systematic audit of the nutritional quality of the generated plans, whose before and after justified a whole new layer of rules in the engine. Still pending, with the environment and the guides already prepared, is a guided test with real nutritionists on the deployed application; its conclusions will close the cycle that began in Phase 1: from the professional to the prototype, from the prototype to the product, and from the product back to the professional.
