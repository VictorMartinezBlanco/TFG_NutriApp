...
\begin{enumerate}
    ...
    \item To build a plan generation engine based on constraint programming techniques, together with a deterministic and independent clinical validator, capable of working on its own without depending on any artificial intelligence component.
...
The technical heart of the project: the formalisation of the diet as a constraint satisfaction problem. After consulting my supervisor, Alejandro, about which approach would be best, I was advised that its implementation should consist of a constraint programming solver and an independent deterministic validator that audits every plan before accepting it, and that I could also add an infeasibility diagnosis for when the requests are incompatible with each other. The engine works without any artificial intelligence component, which made it possible to verify it first with a bank of cases and then from the web itself (by creating a section to modify the parameters used to generate the plan with the engine).
