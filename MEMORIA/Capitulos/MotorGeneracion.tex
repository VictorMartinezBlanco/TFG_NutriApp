...
Este capítulo describe el motor de generación de planes: el componente que, a partir del perfil de un cliente y de un conjunto de restricciones, construye un borrador de plan nutricional que las cumple o explica por qué no existe ninguno que pueda cumplirlas. El motor funciona sin depender de ningún modelo de lenguaje: las restricciones le llegan ya estructuradas, tanto si las escribió el nutricionista en un formulario como si las propuso el traductor del capítulo~\ref{cap:copilotoIA}. Esa independencia se acordó con los tutores y ordenó todo el desarrollo: primero un motor que funciona por sí solo, después la capa de IA por encima.

La sección~\ref{sec:formulacionMotor} plantea el problema y su formulación. Las secciones~\ref{sec:reglasModelo} y~\ref{sec:catalogoClinico} recorren las dos familias de restricciones: las que el modelo aplica siempre y las que configura el profesional. La sección~\ref{sec:objetivoResolucion} recoge la función objetivo y la estrategia de resolución, y las dos últimas cubren el diagnóstico de infactibilidad y el validador que audita cada plan. El cuerpo del capítulo se queda con las decisiones y las ecuaciones que las sustentan; la formalización completa, con las 15 reglas y los 16 tipos uno a uno, está en el Anexo~\ref{apendice:formalizacion}. Las pruebas del motor y las medidas de rendimiento se presentan en el capítulo~\ref{cap:evaluacion}.
...
El resultado es un borrador editable, no un plan cerrado: el nutricionista lo revisa, lo ajusta y decide si lo firma. De ese posicionamiento salen tres propiedades que atraviesan todo el modelo: el plan expone qué restricciones aplicó, el profesional puede relajar las suyas, y cuando el problema no tiene solución el motor explica el conflicto en lugar de fallar sin más.

En la implementación, el motor es una función pura: recibe todos sus datos como argumentos y no consulta la base de datos ni escribe en ella. Esto permite probarlo de forma aislada, con casos construidos a mano, antes de integrarlo en la aplicación. La lectura de datos y la persistencia del plan corresponden a la capa de servicio que lo expone (capítulo~\ref{cap:disenoSistema}).

La salida distingue dos casos disjuntos. Si existe un plan que cumple todas las restricciones obligatorias, se devuelve el plan (días, comidas y alimentos con sus gramos) junto con las métricas de la resolución y los avisos no bloqueantes que se hayan acumulado. Si no existe, se devuelve un diagnóstico: el subconjunto mínimo de restricciones en conflicto, una sugerencia legible y la lista de restricciones que se pueden relajar (sección~\ref{sec:infactibilidad}). Quien llama al motor tiene que tratar los dos casos por separado; no hay forma de confundir un plan con un fallo.
...
El enfoque general se acordó con Alejandro, como comenté en el plan de trabajo: un solver de restricciones que construya el plan y un diagnóstico de infactibilidad cuando no exista. Para implementarlo había tres candidatas razonables: MiniZinc~\citep{minizinc}, un lenguaje de modelado que delega la resolución en distintos solvers; Z3, un solver de satisfacibilidad sobre fórmulas lógicas; y CP-SAT, el solver de programación con restricciones sobre dominios enteros de la biblioteca OR-Tools de Google~\citep{ortools}.

Opté por CP-SAT por dos razones. La primera es de integración: se usa como una biblioteca más de Python, así que el modelo convive con la carga de datos y con el resto del backend en el mismo lenguaje y el mismo proceso, sin un lenguaje de modelado intermedio. La segunda pesó más: CP-SAT permite declarar las restricciones como asunciones y, si el problema no tiene solución, señala qué subconjunto de ellas lo hace imposible. Sobre esa capacidad se construye el diagnóstico de la sección~\ref{sec:infactibilidad}, que convierte el fallo del solver en una explicación útil para el profesional.
...
\begin{itemize}
    ...
    \item $M = \{1, \dots, \texttt{meals\_per\_day}\}$: comidas de un día. Cada comida $m$ lleva asociado un código de franja $c(m)$ (desayuno, comida, cena, tentempiés).
...
Al plantear las variables consideré dos formulaciones. La primera, una única variable entera de gramos por cada terna alimento, día y comida, donde el cero significa que el alimento no está. La segunda, separar la presencia de la cantidad: una variable booleana que dice si el alimento aparece en esa comida y una entera con sus gramos. Elegí la segunda porque gran parte del modelo razona sobre presencia (prohibiciones, variedad, repeticiones) y expresar esas condiciones sobre una booleana es directo; los gramos solo importan cuando el alimento está presente.
...
El mínimo del enlace importa más de lo que parece. En la primera versión bastaba con un gramo para contar como presente, y el solver lo aprovechaba: metía alimentos a 1\,g solo para cumplir la presencia mínima por comida o para sumar variedad barata. Por ello existe un suelo absoluto de presencia de 10\,g, y desde la capa de plausibilidad el mínimo y el máximo reales de cada alimento son los de su perfil de ración (sección~\ref{sec:plausibilidad}).
...
El mismo patrón, multiplicar en cruz en lugar de dividir, resuelve cualquier restricción expresada como porcentaje o cociente; aparecerá varias veces en las secciones siguientes. La resolución resultante, de un gramo y una décima de unidad nutricional, es de sobra para la práctica: ninguna pauta se prescribe con más precisión que esa.
