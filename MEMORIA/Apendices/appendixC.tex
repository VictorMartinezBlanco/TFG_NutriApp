...
Todos los umbrales son constantes con nombre en la configuración del solver. Los cinco campos de \texttt{ObjectiveBands} son las tolerancias de los objetivos blandos de energía y macronutrientes: lo que queda dentro de ellas no penaliza (sección~\ref{sec:anexoObjetivo}).
...
\hline
\texttt{FLOOR\_KCAL} & 1\,200 / 1\,500 & suelo calórico absoluto (mujeres / hombres, R3) \\
\hline
...
\hline
\texttt{ObjectiveBands.kcal\_day} & 50\,kcal & banda diaria del objetivo calórico \\
\hline
\texttt{ObjectiveBands.kcal\_mean} & 25\,kcal & banda de la media del plan, objetivo calórico \\
\hline
\texttt{ObjectiveBands.macro\_day\_pct} & 5\,\% & banda diaria de los macros, sobre el objetivo \\
\hline
\texttt{ObjectiveBands.macro\_mean\_pct} & 2,5\,\% & banda de la media de los macros, sobre el objetivo \\
\hline
\texttt{ObjectiveBands.mean\_factor} & 10 & factor $K$ del ancla de la media \\
\hline
...
\paragraph{\texttt{kcal\_target} (blanda por defecto).} Acerca la energía diaria a un valor $v$. Como blanda, penaliza con el patrón de exceso y defecto de la sección~\ref{sec:anexoObjetivo} solo lo que sobresale de dos bandas: $T_{\text{día}} = 50$\,kcal por día y $T_{\text{media}} = 25$\,kcal sobre la media del plan, esta con un factor $K = 10$ que la hace casi obligatoria:
\begin{gather*}
...
\text{penalización} \;=\; w_{\text{kcal}} \cdot \text{peso de la fila} \cdot \Big( \sum_{d \in D} \text{exceso\_diario}_d \;+\; K \cdot \text{exceso\_media} \Big)
\end{gather*}
Como dura:
\begin{equation*}
...
\paragraph{\texttt{macro\_target} (blanda por defecto).} Igual que el objetivo calórico pero sobre un macronutriente (proteína, hidratos o grasa), con las bandas en porcentaje del objetivo: un 5\,\% por día y un 2,5\,\% en la media. El peso de familia se elige según el macro.

\paragraph{\texttt{nutrient\_min} (configurable).} Mínimo diario de un nutriente. Como dura, para todo día, $\text{nut}_{d,n} \ge \text{scale\_target}(v)$. Como blanda, penaliza solo el defecto, con una variable por día:
\begin{equation*}
...
\paragraph{\texttt{forbid\_food} (dura).} Prohíbe un alimento concreto:
\begin{equation*}
\text{ha\_consumido}[f_0,d,m] = 0, \qquad \forall d \in D,\ m \in M
\end{equation*}

\paragraph{\texttt{forbid\_tag} (dura).} Prohíbe todos los alimentos de una etiqueta; es el mecanismo de las alergias e intolerancias:
\begin{equation*}
...
\paragraph{\texttt{meal\_kcal\_ratio} (blanda).} Reparte la energía diaria entre las comidas según porcentajes deseados. Para cada comida con porcentaje objetivo $pct$ (escalado por 10 para conservar un decimal), la desviación se linealiza con la multiplicación cruzada:
\begin{equation*}
...
\paragraph{\texttt{forbid\_combination} (dura).} Prohíbe que dos alimentos, o un alimento y una etiqueta, coincidan en la misma comida. Con indicadores de presencia de cada grupo en la comida ($a$ y $b$, máximos de cada grupo):
\begin{equation*}
...
Los objetivos blandos de energía y macronutrientes no penalizan $\text{over} + \text{under}$ directamente: sobre ellos se aplican las dos bandas de tolerancia de la sección~\ref{sec:anexoTipos}, la diaria y la de la media del plan, y solo los excesos fuera de ellas entran en la función objetivo, el de la media multiplicado por $K$. Con las tolerancias a cero la formulación se reduce a la desviación absoluta total sin variables adicionales. Los mínimos y máximos de nutriente no llevan banda porque son umbrales de un solo lado.
...
Un objetivo calórico con peso de fila 7 pesa $10 \cdot 7 = 70$ por unidad escalada de desviación fuera de la banda diaria ($70\,000$ por kcal, y $700\,000$ por cada kcal que la suma de las desviaciones del plan se salga de la banda de la media, por el factor $K$); una preferencia de familia con peso 4 pesa $4\,000 \cdot 4 = 16\,000$ por aparición. En unidades naturales son magnitudes comparables, por el factor de $1\,000$ de la sección~\ref{sec:anexoUmbrales}. Dentro de las bandas la energía no cuesta nada, así que ahí deciden la estructura y las preferencias.
...
\hline
\texttt{kcal\_target} / \texttt{macro\_target} & \texttt{catalog.\_h\_kcal\_target}, \texttt{\_h\_macro\_target}, \texttt{\_target\_deviation}; \texttt{config.ObjectiveBands} \\
\hline
...
\hline
Linealización del valor absoluto & \texttt{catalog.\_target\_deviation} \\
\hline
